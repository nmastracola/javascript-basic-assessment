...
var secondHero = heroes.splice(2,0);
...
function doubleTheFun (param) {
	if (typeof param === "string" && isNaN(param) === true ) {
        return param+param
    } else if (typeof param === "string" && isNaN(param) === false) {
...
function makeChatMessage (msg, aut) {
	return {
		message: msg,
		author: aut,
		timestamp: new Date()
...
function coderTest (person) {
	for (var prop in person) {
		if (person[prop] === 'Jeremy') {
			person.lovesCode = 10;
		} else if (person[prop] === "Brack") {
			person.lovesCode = 0;
		} else {
			person.lovesCode = 5;
		}
